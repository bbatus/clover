...
| R11 | 18.09 ve 19.09 işleri için SonarQube taraması yapılmadı (AGENTS.md "büyük değişiklik sonrası Sonar" kuralı). Trivy 19.09.2026'da yapıldı (#71). | Açık — süreç | Sonar için her repoda `.sonar-token` yok; SonarQube arayüzünden (My Account → Security) token üretilip `docker compose -f ../vodafonepaycomtr/tools/sonarqube/docker-compose.yml up -d` + `SONAR_TOKEN=… scripts/sonar-scan.sh` çalıştırılmalı. |
...
| R14 | ~~Yerel Docker Desktop Docker Hub'a ulaşamıyordu.~~ | KAPANDI 19.09.2026 (#71) | Kullanıcı Docker Desktop'ı yeniden başlattı. İlk kurulumda disk doldu ("no space left on device"); yalnız build önbelleği ve sahipsiz eski node imajı temizlendi (10 GB). Diğer projelerin imaj ve volume'larına dokunulmadı. İki container yeni kodla ayakta. Not: imaj kurarken ikisini aynı anda değil sırayla kurun. |
...
| R20 | Trivy: Payload CVE-2026-11779 (orta düzey; varsayılan "hesap kilidini aç" yetkisi her giriş yapmış kullanıcıya açık). Yamalı sürüm yok (clover 3.88.0, site 3.87.0). | Bilgi — bizde etkisiz | `Users.ts` → `access.unlock` yalnız New Vertical Maker'a açık (`accountLock.test.ts` ile sabit), yani varsayılan kullanılmıyor. Payload yamalı sürüm yayınlayınca yükseltilmeli (3.90.1 var ama Trivy onu da düzeltmiş saymıyor). |
