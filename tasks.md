...
**15.09.2026 — bu dosya buraya taşındı.** Önceden monorepo'da (`vodafonepaycomtr/`
klasörü, ortak dosyaların bulunduğu kök) tutuluyordu; artık monorepo aktif
geliştirme kapsamında değil (bkz. o repodaki AGENTS.md), her repo kendi log'unu
kendi tutuyor. Aşağıdaki içerik, `clover/` henüz `cms/` adıyla monorepo'nun bir
alt klasörüyken (01.09.2026'daki repo ayrımına kadar) yürütülen TÜM işin —
çoğunlukla Payload/CMS tarafı, kimi zaman site render'ını da etkileyen — birebir
korunmuş geçmişi. Site'a özel işlerin kendi log'u artık
`vodafonepaycomtr-site/tasks.md`'de tutuluyor; 01.09.2026'dan sonraki maddelerde
(39+) hangi repoyu etkilediği madde başlığından/içeriğinden zaten belli.
...
## 64. Son beş işin production gözden geçirmesi (18.09.2026)

- Kullanıcı çevrimdışıyken #59–#63 (zamanlanmış yayın, SEO asistanı, önizleme linki, kırık linkler, toplu işlemler) regüle bir e-para kuruluşunun canlı ürünü gözüyle gözden geçirildi: doğruluk ve uç durumlar, güvenlik (yetki, sır, SSRF, kötüye kullanım), maker→checker bütünlüğü, operasyon, i18n, testler. Site tarafı: vodafonepaycomtr-site #64-site.
- **Bulgular → yapılan / bırakılan:**
  - **Zamanlayıcı yarışı (maker→checker delinmesi) → düzeltildi.** Zamanlayıcı önce "zamanı gelenler"i sorguluyor, sonra tek tek yayınlıyordu; context bayrağı `manageCampaignSchedule`'ı ve `denyMakerPublish`'i atlıyor, `update` de EN YENİ sürümü yayınlıyor. Arada bir Maker değişiklik kaydederse (onay "İncelemede"ye düşer) o onaysız değişiklik canlıya çıkabilirdi. Artık hook, zamanlayıcının kendi kaydında `originalDoc`'u (aynı kayıt içinde okunuyor) yeniden kontrol ediyor: hâlâ "scheduled", hâlâ taslak ve zamanı gelmiş değilse `ScheduleNoLongerDueError` ile reddediyor; zamanlayıcı bunu hata değil "atlandı" sayıyor. 2 test.
  - **Yayınlanamayan planlı kampanya → düzeltildi.** Onaydan sonra bozulan bir kayıt (ör. silinen kategori/görsel — AGENTS.md'deki FAQ kategori tuzağının aynısı) 30 sn'de bir sonsuza dek deneniyor ve her seferinde ERROR logluyordu; CMS'te kimse nedenini göremiyordu. Artık: 30 sn'den başlayıp 1 saate kadar katlanan bekleme (pod başına bellekte), ilk hata Denetim Kayıtları'na "sistem (zamanlanmış yayın)" aktörüyle "zamanlanmış yayını BAŞARISIZ oldu … — {sunucu mesajı}" olarak yazılıyor, kampanya ekranında plan saatinden 2 dk sonra hâlâ planlıysa turuncu "Yayınlanamadı: {tarih} planı geçti — nedeni Denetim Kayıtları'nda" notu çıkıyor (tr/en, `roleAwarePublishButton.scheduleOverdue`). Sorgu artık `scheduledPublishAt`'e göre sıralı (en eski plan önce). 3 test.
  - **Advisory lock bağlantı havuzunda takılı kalabilirdi → düzeltildi.** Kilit oturum düzeyinde; `pg_advisory_unlock` hata verirse bağlantı havuza kilitli haliyle dönüyor ve diğer pod hiçbir turda kilidi alamıyordu. Artık kilit/kilit açma hatasında bağlantı `release(err)` ile yok ediliyor; Postgres kilidi oturumla birlikte bırakıyor (pod'un iş ortasında ölmesiyle aynı). Pod ölürse: oturum kapanır, kilit düşer, kampanya bir sonraki turda (başka pod'da) yayınlanır; çift yayın sorgu + yeni yeniden-kontrolle engelli.
  - **Revalidate çağrısında zaman aşımı yoktu → düzeltildi** (`src/hooks/revalidate.ts`, 5 sn). Her kaydın afterChange'i, zamanlayıcı ve toplu işlemin her kaydı siteye bu çağrıyı yapıyor; site pod'u asılırsa hepsi süresiz bekliyordu.
  - **Kırık Linkler SSRF → düzeltildi.** Dış link kontrolü içerikteki (her editör rolünün yazabildiği) adrese küme içinden istek atıyordu; `http://169.254.169.254/…`, `http://10.x:5432`, Service adları ile iç ağ taranabilirdi (NetworkPolicy egress hâlâ `{}`). Artık adres çözülüyor; loopback / özel / link-local / CGNAT / ULA / multicast / ayrılmış bir IP'ye çıkan (ya da çözülmeyen) hedefe istek atılmıyor, satırda "İç ağ adresi — ziyaretçiler açamaz, kontrol edilmedi" yazıyor. Site içi linkler yapılandırmadaki adrese gittiği için etkilenmedi. Kalan risk: çöz-sonra-bağlan arasında DNS rebinding (tam çözüm sabit IP'li dispatcher; aşağıda). 4 test.
  - **Tarama 2000 kayıtta sessizce kesiliyordu → düzeltildi.** `pagination: false` ile Payload `limit: 2000`'i yine uyguluyor (drizzle findMany kontrol edildi); 2000'i aşan koleksiyonun kalanı taranmadan "kırık yok" diyordu. Artık 500'lük sayfalarla hepsi okunuyor. 1 test. Aynı pod'da aynı anda tek tarama (ikincisi 429 + Türkçe mesaj; arayüz sunucunun mesajını gösteriyor).
  - **404 kayıtları: kalıcı dolma → düzeltildi.** 5000 satır sınırı dolunca bir daha hiçbir yeni 404 kaydedilmiyordu ve satır silmenin yolu yoktu (tek bir tarayıcı botu raporu kalıcı olarak öldürebilirdi). Artık sınırdayken en uzun süredir görülmeyen, tek seferlik, yok sayılmamış adres yer açıyor; birden çok görülenler ve editörün işaretledikleri bu yolla silinmiyor.
  - **404 kayıtları: referrer'da kişisel veri → düzeltildi.** Geldiği sayfa ham saklanıyordu; başka siteden gelenlerin sorgu dizesi (e-posta, kampanya/arama parametresi, token) tüm CMS kullanıcılarına görünüyordu. Artık yalnızca http(s) origin + path (CMS'te ve sitede ayrı ayrı).
  - **"Yok say" denetim kaydı yazmıyordu → düzeltildi** (adresle birlikte).
  - **SEO asistanı Sayfa Meta'yı okumuyordu → düzeltildi.** Site `[...slug]` ve `/` için başlık/açıklama sırası: sayfanın kendi SEO alanı → yayındaki Sayfa Meta Bilgileri satırı → varsayılan. Asistan ortadakini atlıyordu; yerelde anasayfa ve "Ulaşım Ödemeleri" (id 17) için panel sitenin hiç göstermediği bir başlığı puanlıyordu. Artık Sayfalar'da aynı adresin yayındaki Sayfa Meta satırı okunuyor, kontrol metni "Sayfa Meta Bilgileri'ndeki başlık kullanılacak" diyor. Sitenin `<title>`'ı ile karşılaştırıldı, aynı. 1 test. (Paylaşım görselinde Sayfa Meta yedeği hâlâ taklit edilmiyor, küçük; bırakıldı.)
  - **Kontrol edilip sorun bulunmayanlar:**
    - Yeni uç noktaların hepsi anonimken (otomatik giriş KAPALI yeniden başlatılarak) 401/403: bulk-actions, broken-links/scan, share-links create/revoke/resolve, not-found-hits record/ignore; share-links ve not-found-hits listeleri 403; `?draft=true` 403.
    - `tokenHash` REST'te sorgulanamıyor ("The following path cannot be queried"), listede yok. Anahtar 24 bayt rastgele, yalnızca SHA-256'sı saklanıyor, PREVIEW_SECRET karşılaştırması sabit zamanlı.
    - Toplu işlem: 10 kampanya toplu yayın ~0,3 sn (yerel) — 100 kayıt sınırında OpenShift route'unun 30 sn varsayılan zaman aşımı sorun değil. Önkontrol yalnızca atlama ekliyor; asıl yetki hâlâ kayıt başına hook'larda.
    - Zamanlayıcı pod açılışında başlıyor: probe'lar (`/api/health/*`) Payload uç noktası, onInit'i tetikliyor. Log gürültüsü yok (yalnızca yayın/hata anında satır).
    - Tüm yeni admin bileşenleri `useAdminLocale` ile tr/en. Migration dosyası (§11–§15) iki kez üst üste çalıştırıldı, zincir dev DB ile birebir aynı (12.434 satır, fark 0).
- **Karar gerektirenler (uygulanmadı):**
  1. Editörlerin kendi "Önizle" linki PREVIEW_SECRET'ı URL'de taşıyor (tarayıcı geçmişi, proxy logları) ve önizleme bandı da HTML'e gömüyor. Bu sırla CMS REST'ten tüm taslaklar okunabiliyor, 404 kaydı yazılabiliyor, önizleme linkleri çözülebiliyor. Öneri: editör başına kısa ömürlü imzalı önizleme anahtarı. (Eskiden beri var; #61'de bilinçli olarak paylaşım linkine taşınmadı.)
  2. `k8s/networkpolicy.yaml` egress'i hâlâ `{}`. SSRF kodda kapatıldı ama asıl koruma egress'i Postgres + MinIO + site Service + DNS (+ istenirse dış link kontrolü için proxy) ile sınırlamak. Ağ ekibiyle.
  3. Toplu yayında Checker kayıtları tek tek önizlemeden onaylıyor. Kampanyalar için toplu "Yayınla"nın kapatılıp tek tek incelemeye zorlanması istenebilir (dört göz ilkesi açısından ürün kararı).
  4. Growth Maker, Checker onayı almamış bir taslak için dışarıya önizleme linki üretebiliyor (tasarım gereği: hukuk/pazarlama incelemesi için). Sadece Checker'ların üretmesi istenirse tek satırlık değişiklik.
  5. Önizleme linkinin "görüntülenme" sayısına sohbet uygulamalarının link önizleme botları da giriyor (Teams/Slack'e yapıştırınca). Denetim için bilinmeli.
  6. Saklama süresi (KVKK): `not_found_hits`, `share_links` ve toplu işlem özet satırları için bir saklama/temizleme politikası yok.
  7. Hız sınırları ve zamanlayıcının geri çekilme durumu pod başına bellekte (HPA ile 2 pod → sınırlar iki katı). Kesin sınır gerekiyorsa Postgres/Redis tabanlı sayaç.
  8. Kampanya bitiş günü İstanbul gece yarısında dolduğunda sitede o anda hiçbir revalidate tetiklenmiyor; liste sayfası ISR penceresi (fetch `revalidate: 3600`) kadar eski kalabiliyor. İstenirse zamanlayıcı gece yarısı `campaigns` etiketini tazeleyebilir.
  9. DNS rebinding (bkz. SSRF maddesi): tam kapatmak için dış kontrolde çözülen IP'ye sabitlenmiş bir undici dispatcher.
- Değişen dosyalar: `src/lib/campaignSchedule.ts`, `src/components/RoleAwarePublishButton.tsx`, `src/lib/translationDefaults.ts`, `src/styles/custom.css`, `src/hooks/revalidate.ts`, `src/lib/brokenLinks.ts`, `src/components/BrokenLinksApp.tsx`, `src/collections/NotFoundHits.ts`, `src/lib/seoAnalysis.ts`, `src/components/SeoAssistant.tsx`, `k8s/configmap.yaml` (yalnızca yorum: `SITE_INTERNAL_URL`, `SCHEDULED_PUBLISH_DISABLED` belgelendi), `CLAUDE.md` (bekleyen deploy adımlarına not).
- **Doğrulama (dev sunucusu, headless Chrome 1440px):**
  - İki planlı kampanya oluşturulup onaylandı; birinin açıklaması DB'de silinerek "onaydan sonra bozulmuş kayıt" taklit edildi. Sağlam olan planlanan dakikada yayına girdi. Bozuk olan "attempt 1, next try in 30s" → "attempt 2, next try in 60s" ile geri çekildi. Denetim Kayıtları'nda tek "BAŞARISIZ … Açıklama" satırı var. Kampanya ekranındaki turuncu not ekran görüntüsüyle kontrol edildi.
  - Kırık Linkler canlı tarama: 24 link, kırık yok (sayfalı okuma öncekiyle aynı sonucu verdi); eşzamanlı ikinci tarama 429.
  - Site üzerinden 404 bildirimi: çapraz site istek kaydedilmedi; aynı kökenli istekte referrer `https://mail.example.com/r` olarak (sorgusuz) kaydedildi.
  - SEO asistanı, Sayfa Meta'lı sayfa: panel başlığı sitenin `<title>`'ıyla aynı ("Ulaşım Ödemeleri | Vodafone Pay ile Bakiye Yükle").
...
## 66. Güvenlik ve dayanıklılık: rate limit, güvenlik başlıkları, oturum zaman aşımı, JSON log, imzalı önizleme (19.09.2026)

Kullanıcının seçtiği "1 · Güvenlik & dayanıklılık" isteği. Site tarafı: `vodafonepaycomtr-site/tasks.md` #66-site.

- **API hız sınırı (pod'lar arası ortak):** `src/lib/rateLimit.ts` + `src/lib/apiGuard.ts`. REST (`api/[...slug]`) ve GraphQL route handler'ları `withApiGuard` ile sarıldı; giriş, toplu işlem, kırık link taraması ve önizleme linki uç noktaları da REST üzerinden geldiği için kapsamda. Kurallar (60 sn pencere): giriş 10/IP, şifre sıfırlama/kilit açma 5/IP, link taraması 3, toplu işlem 10, önizleme linki 20, GraphQL 120 (girişsiz 60), okuma 600 (girişsiz 300), yazma 120 — oturum başına, oturum yoksa IP başına. Sayaç `clover_ops.rate_limit_buckets` tablosunda tek atomik `INSERT … ON CONFLICT` ile. Aşımda 429 + Türkçe mesaj + `Retry-After`; aşımın ilk isteği Denetim Kayıtları'na "API istek sınırı aşıldı" diye yazılıyor. Sitenin sunucudan sunucuya çağrıları `x-internal-auth` (REVALIDATE_SECRET) ile muaf. IP, X-Forwarded-For'un güvenilen ucundan (`TRUSTED_PROXY_HOPS`), baştaki taklit edilebilir değerden değil. DB ulaşılamazsa istek geçiyor (fail-open). `RATE_LIMIT_DISABLED`, `RATE_LIMIT_MULTIPLIER` ayarları var.
- **Güvenlik başlıkları:** `src/lib/security/headers.ts` + `src/proxy.ts`: CSP (dış script yok, `frame-ancestors 'none'`, `frame-src` yalnız site), HSTS (https'te), `X-Frame-Options: DENY`, nosniff, Referrer-Policy, Permissions-Policy, COOP. SEO Dosyaları'ndaki robots/llms alanları `code` → `textarea` (Monaco editörü cdn.jsdelivr.net'ten yükleniyordu; CSP'yi o CDN için açmak yerine). Kolon tipi aynı, şema değişmedi.
- **Oturum:** `Users.auth.tokenExpiration` 12 saat → 30 dk hareketsizlik (`CMS_SESSION_IDLE_MINUTES`). Payload admin'de hareket oldukça token'ı yeniliyor ve süre dolmadan kendi "oturumda kal" penceresini gösteriyor; süre dolunca `/admin/logout-inactivity`'e atıyor.
- **İstek kimliği + JSON log:** her yanıtta `x-request-id` (gelen varsa korunur). Production'da Payload logger JSON (`{"time","service":"clover","level","msg"…}`), API erişim logu da aynı biçimde (yazmalar, hatalar, 429'lar ve 1 sn'den yavaş istekler; `API_ACCESS_LOG=all` ile hepsi). `LOG_FORMAT=pretty` eski görünüm.
- **R1 kapandı:** `src/lib/preview.ts` "Önizle" linki artık `?token=<base64url({p,e}).hmac>` — tek sayfaya bağlı, 2 saat geçerli. `PREVIEW_SECRET` URL'de yok.
- Testler: 697/697 (yeni: rateLimit 9, apiGuard 5, securityHeaders, preview 3, oturum süresi 1); tsc 0, eslint 0.
