...
import { REFERENCE_MAP } from "./referenceMap";
import type { ReferenceSource } from "./referenceMap";

/**
 * `ReferenceSource`/`REFERENCE_MAP` now live in referenceMap.ts (no "payload"
 * import) so client components can read the map without pulling this file's
 * server-only hook logic into the browser bundle — see that file's comment.
 * Re-exported here so every existing importer of "@/hooks/referentialIntegrity"
 * keeps working unchanged.
 */
export { REFERENCE_MAP } from "./referenceMap";
export type { ReferenceSource } from "./referenceMap";
