...
/**
 * ROLES/ROLE_OPTIONS/RoleValue live in roleConstants.ts (no "payload" import)
 * so client components can import them without pulling this file's
 * server-only dependencies into the browser bundle — see that file's own
 * comment. Re-exported here so every existing server-side importer of
 * "@/access/roles" keeps working unchanged.
 */
export { ROLES, ROLE_OPTIONS } from "./roleConstants";
export type { RoleValue } from "./roleConstants";
